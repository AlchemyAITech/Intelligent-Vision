...
* *New!* **🚀 三合一全功能集群 (v2026.58)**:
  ...
  * *Hot!* **多实体裂变引擎**: 新增带有灵敏度滑块的【一键同类全图扩展】与【文本群相搜索】功能。允许单次交互后截获多个同类像素掩码，并在确认入库时执行**后台解包裂变**，智能将数组分散化为独立记录集，大幅削减零碎的多目标追踪建组成本。
...
项目已全面迁移至 FastAPI + 原生 Vue3 架构。请终端处切换到 `codev2` 根目录下，并严格按以下无坑流程启动：

### 1. 挂载环境与依赖对齐
```bash
# 激活内置的虚拟隔离环境 (Mac/Linux)
source .venv/bin/activate

# 若有依赖报警，随时可通过该命令补齐最新包
pip install -r requirements.txt
```

### 2. 清理残存并拉起服务
为避免开发过程中因端口持续被前个进程占用导致的 502/404 故障，请使用下述组合命令**一键清理并启动**：
```bash
# 清空 8000 端口占用并清理僵死进程
lsof -t -i :8000 | xargs kill -9 2>/dev/null; pkill -f uvicorn 2>/dev/null;

# 启动高并发后台服务
python -m uvicorn src.main:app --host 0.0.0.0 --port 8000
```

当终端出现 `Application startup complete.` 后，直接打开浏览器访问：
👉 **[http://127.0.0.1:8000](http://127.0.0.1:8000)** 即可开启全部实验室。
