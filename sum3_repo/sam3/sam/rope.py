...
def apply_rotary_enc(
    ...
    if repeat_freqs_k:
        ...
        # MPS Backend bypass: repeat() is not supported for complex tensors!
        freqs_cis_real = torch.view_as_real(freqs_cis)
        freqs_cis = torch.view_as_complex(freqs_cis_real.repeat(*([1] * (freqs_cis.ndim - 2)), r, 1, 1))
...
def apply_rotary_enc_real(
    ...
    freqs_cis_real = reshape_for_broadcast(freqs_cis_real, xq_real).to(xq.device)
    freqs_cis_imag = reshape_for_broadcast(freqs_cis_imag, xq_imag).to(xq.device)
