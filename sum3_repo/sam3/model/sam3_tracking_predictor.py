...
class Sam3TrackerPredictor(Sam3TrackerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Determine auto-precision dtype based on device
        self.amp_dtype = torch.bfloat16 if (torch.cuda.is_available() or (hasattr(torch, 'mps') and torch.backends.mps.is_available())) else torch.float32
    ...
    def add_new_points_or_box(
        ...
        if self.iter_use_prev_mask_pred:
            ...
            if prev_out is not None and prev_out["pred_masks"] is not None:
                prev_sam_mask_logits = prev_out["pred_masks"].to(device=self.device)
    ...
    def _consolidate_temp_output_across_obj(
        ...
        if run_mem_encoder:
            ...
            high_res_masks = torch.nn.functional.interpolate(
                consolidated_out["pred_masks"].to(device),
