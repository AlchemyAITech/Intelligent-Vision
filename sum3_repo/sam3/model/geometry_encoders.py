...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_pool_project is not None:
            ...
            grid = (grid * 2) - 1
            if n_points == 0:
                sampled = torch.empty(bs, self.d_model, n_points, 1, device=img_feats.device, dtype=img_feats.dtype)
            else:
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_pool_project is not None:
            ...
            # Apple Silicon / MPS does not support setting storage of explicitly pin-memmory mapped cpu tensors to MPS
            # Therefore we just move it to the device directly without pin_memory()
            scale = scale.to(device=boxes_xyxy.device, non_blocking=True)
            scale = scale.view(1, 1, 4)
            boxes_xyxy = boxes_xyxy * scale
            if n_boxes == 0:
                sampled = torch.empty(bs * n_boxes, self.d_model, self.roi_size, self.roi_size, device=img_feats.device, dtype=img_feats.dtype)
            else:
