...
try:
    ...
    class MockTriton:
        ...
        @staticmethod
        def autotune(*args, **kwargs): return lambda func: func
        @staticmethod
        def Config(*args, **kwargs): return None
    class MockTl:
        constexpr = any
    triton = MockTriton()
    tl = MockTl()
...
def edt_triton(data: torch.Tensor):
    ...
    FALLBACK for Mac/CPU: Uses OpenCV distanceTransform.
    """
    assert data.dim() == 3
    ...
    if not data.is_cuda:
        ...
        for b in range(B):
            # OpenCV expects 0 for background, 255 for foreground (or vice versa depending on logic)
            # cv2.distanceTransform calculates distance to zero pixels.
            # So if we want distance to boundary of "1" regions, we need to invert if needed.
            # Official SAM3 logic: "Implicitly the function is 0 if data[i,j]==0 else +infinity"
            # This means it calculates distance from pixels where data==1 to the nearest data==0 pixel.
            dist = cv2.distanceTransform(data_np[b].astype(np.uint8), cv2.DIST_L2, 0)
            output_np[b] = dist
        return torch.from_numpy(output_np).to(data.device)

    # Original Triton Logic (kept for CUDA compatibility)
    try:
        import triton
        import triton.language as tl
    except ImportError:
        # If CUDA but no triton, fallback to CPU
        return edt_triton(data.cpu()).to(data.device)
