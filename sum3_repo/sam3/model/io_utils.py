...
valid_device = torch.device(device_type)
...
def load_image_as_single_frame_video(
    ...
    if not offload_video_to_cpu:
        images = images.to(valid_device)
        img_mean = img_mean.to(valid_device)
        img_std = img_std.to(valid_device)
    # normalize by mean and std
    images -= img_mean
    images /= img_std
...
def load_video_frames_from_video_file_using_cv2(
    ...
    if not offload_video_to_cpu:
        video_tensor = video_tensor.to(valid_device)
        img_mean = img_mean.to(valid_device)
        img_std = img_std.to(valid_device)
    # normalize by mean and std
