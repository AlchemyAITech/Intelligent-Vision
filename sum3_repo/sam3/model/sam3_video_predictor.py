...
class Sam3VideoPredictor:
    ...
    def _get_session_stats(self):
        ...
        if torch.cuda.is_available():
            ...
            gpu_str = f", GPU memory: {mem_allocated} MiB used and {mem_reserved} MiB reserved (max over time: {max_allocated} MiB used and {max_reserved} MiB reserved)"
        else:
            gpu_str = ", no CUDA memory tracking available"
            
        session_stats_str = f"live sessions: [{', '.join(live_session_strs)}]{gpu_str}"
        return session_stats_str

    def _get_torch_and_gpu_properties(self):
        """Get a string for PyTorch and GPU properties (for logging and debugging)."""
        if torch.cuda.is_available():
            gpu_info = f"GPU device: {torch.cuda.get_device_properties(torch.cuda.current_device())}"
            arch_list = torch.cuda.get_arch_list()
        else:
            gpu_info = "GPU device: MPS/CPU"
            arch_list = "N/A"
            
        torch_and_gpu_str = (
            f"torch: {torch.__version__} with CUDA arch {arch_list}, "
            f"{gpu_info}"
        )
